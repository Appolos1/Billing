/**
 * billing login — authenticate via browser or API key
 */
...
module.exports = function(program) {
  program
    .command('login')
    .description('Log in to your Billing account')
...
async function loginWithKey(key) {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      console.log(clr('dim', `\n  Run ${clr('cyan', 'billing start')} to begin coding.\n`));
    } else {
...
async function loginInteractive() {
  ...
  console.log(clr('cyan', '  Enter your Billing credentials\n'));
  ...
  try {
    ...
    console.error(clr('dim', `    Make sure the Billing server is running at ${getServerUrl()}`));
    process.exit(1);
  }
}
